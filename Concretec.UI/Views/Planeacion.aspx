            ...
            <body>
                <form id="form1" runat="server">
                    ...
                    <table id="t1" border="0px">
                        <% AgenteUnidades au=new AgenteUnidades(); List<ConsultaUnidad> planta = new List
                            <ConsultaUnidad>();
                                int saltos = 0;
                                int numviaje = 0;
                                ...
                                where pp.CveCiudad == DatosUsuario.Ciudad && pp.CveDosificadora.Contains("PD") &&
                                pp.IDEstatus == 1
                                orderby pp.Planta
                                ...
                                for (int j = 0; j <= (valstr.Length - 2); j++) { valores[j]=int.Parse(valstr[j]); } if
                                    (valores[0].ToString()=="-1" ) { plantas=from pp in planta where
                                    pp.CveCiudad==DatosUsuario.Ciudad && pp.CveDosificadora.Contains("PD") &&
                                    pp.IDEstatus==1 orderby pp.Planta select new { pp.Planta, pp.IDClaveUnidad }; } else
                                    { plantas=from pp in planta where pp.CveCiudad==DatosUsuario.Ciudad &&
                                    pp.CveDosificadora.Contains("PD") && pp.IDEstatus==1 &&
                                    valores.Contains(pp.IdPlanta) orderby pp.Planta select new { pp.Planta,
                                    pp.IDClaveUnidad }; } } List<ConsultaUnidad> UnidadesOcupadas = new List
                                    <ConsultaUnidad>();
                                        ...
                                        Response.Write("<THEAD>
                                            <tr>");
                                                ...
                                                for (int i = int.Parse(HoraInicio.Substring(0, 2)); i <=
                                                    int.Parse(HoraFin.Substring(0, 2)); i++) { _hora=i.ToString(); if
                                                    (_hora.Length==1) { _hora="0" + _hora; } for (int ii=0; ii <=50;
                                                    ii=ii + 10) { _html.Clear(); _minutos=ii.ToString(); if
                                                    (_minutos.Length==1) { _minutos="0" + _minutos; } _horaminutos=_hora
                                                    + _minutos; _html.Append("<td class='tdgris2'
                                                    id=" + _horaminutos + ">");
                                        ...
                                        <TBODY>
                                            <tr>
                                                <td class='tdgris'></td>
                                                <td class='tdgris' colspan='12'></td>
                                            </tr>");

                                            au = new AgenteUnidades();
                                            UnidadesOcupadas = new List<ConsultaUnidad>();
                                                dia = Request.QueryString["Fecha"];
                                                UnidadesOcupadas = au.ObtenerUnidadesOcupadas(DatosUsuario.Ciudad,
                                                DateTime.Parse(dia));
                                                ...
                                                where _unidad.Planta == p.Planta && _unidad.IDClaveUnidad ==
                                                p.IDClaveUnidad
                                                ...
                                                Response.Write("<tr>");
                                                    ...
                                                    Response.Write("<td class='tdgris'>" + p.Planta.Substring(0,3) + " "
                                                        + p.IDClaveUnidad + "</td>");
                                                    }
                                                    else
                                                    {
                                                    ...
                                                    for (int i = int.Parse(HoraInicio.Substring(0, 2)); i <=
                                                        int.Parse(HoraFin.Substring(0, 2)); i++) { _hora=i.ToString();
                                                        if (_hora.Length==1) { _hora="0" + _hora; } for (int ii=0; ii
                                                        <=50; ii=ii + 10) { _html.Clear(); _minutos=ii.ToString(); if
                                                        (_minutos.Length==1) { _minutos="0" + _minutos; }
                                                        _horaminutos=_hora + _minutos; if (cuantos> 0)
                                                        {
                                                        var _unidades_ = from _unidad in UnidadesOcupadas
                                                        where _unidad.Planta == p.Planta && _unidad.IDClaveUnidad ==
                                                        p.IDClaveUnidad && _unidad.horaminutos_inicio.Replace(":", "")
                                                        == _horaminutos
                                                        select new
                                                        {
                                                        _unidad.CveCiudad,
                                                        ...
                                                        {
                                                        case "Despachado":
                                                        _html.Append("<td id=" + _horaminutos + " class='vdespachado'
                                                            colspan='" + Entidad.bloques.ToString() + "'><a href='#'
                                                                ...
                                                                case "Cancelado":
                                                                _html.Append("
                                                        <td id=" + _horaminutos + " class='vcancelado'
                                                            colspan='" + Entidad.bloques.ToString() + "'><a href='#'
                                                                STYLE='TEXT-DECORATION: NONE;color:Black;'
                                                                class='tooltip'>");
                                                                break;
                                                                ...
                                                                case "Programado":
                                                                _html.Append("
                                                        <td id='" + _horaminutos + "' class='vprogramado'
                                                            colspan='" + Entidad.bloques.ToString() + "'><a href='#'
                                                                STYLE='TEXT-DECORATION: NONE;color:Black;'
                                                                class='tooltip'>");
                                                                break;
                                                                ...
                                                                case "Entregado":
                                                                _html.Append("
                                                        <td id=" + _horaminutos + " class='vcerrado'
                                                            colspan='" + Entidad.bloques.ToString() + "'><a href='#'
                                                                STYLE='TEXT-DECORATION: NONE;color:Black;'
                                                                class='tooltip'>");
                                                                break;
                                                        ...
                                                        <td id=" + _horaminutos + " class='vprogramado'
                                                            colspan='" + Entidad.bloques.ToString() + "'><a href='#'
                                                                ...
                                                                {
                                                                _html.Append(
                                                                "<div>Viaje : " + Entidad.numviaje.ToString() + "
                                                                    Pedido: "
                                                            ...
                                                            {
                                                            saltos = saltos - 1;
                                                            if (saltos < 1) { _html.Append("<td id=" + _horaminutos + "
                                                                class='tdverde'>");
                                                                }
                                                                ...
                                                                {
                                                                saltos = saltos - 1;
                                                                if (saltos < 1) { _html.Append("<td
                                                                    id=" + _horaminutos + " class='tdverde'>");
                                                                    }
                                                                    }
