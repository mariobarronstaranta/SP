...
j<g.length;
j++){f=g[j];
if(this._enableDragDrop){f.style.cursor="move";
}var n=c[f.id];
...
}}else{if(this.get_isDeferedLayoutUpdate()&&(d.className.indexOf("rpgSortAsc")!=-1||d.className.indexOf("rpgSortDesc")!=-1)){var g=h._getField(d.parentNode);
if(g){if(d.className.indexOf("rpgSortAsc")!=-1){Sys.UI.DomElement.removeCssClass(d,"rpgSortAsc");
Sys.UI.DomElement.addCssClass(d,"rpgSortDesc");
}else{Sys.UI.DomElement.removeCssClass(d,"rpgSortDesc");
Sys.UI.DomElement.addCssClass(d,"rpgSortAsc");
}this._sortField(g);
f.stopPropagation();
f.preventDefault();
return false;
