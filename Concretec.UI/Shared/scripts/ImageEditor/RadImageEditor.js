...
if(!this._isDialogMoved||(c.get_top()==0&&c.get_left()==0)){var g=this.get_location();
var k=$telerik.getClientBounds();
var d=270;
var h=Math.max(document.documentElement.scrollLeft,document.body.scrollLeft);
var e=k.width+h;
var f=this._isRightToLeft?g.x-280:g.x+470;
if(e-(f)>d){c.set_left(f);
}else{c.set_left(h+10);
}var i=Math.max(document.documentElement.scrollTop,document.body.scrollTop);
var j=(g.y+70>i)?g.y+70:i+10;
c.set_top(j);
}c.set_closed(false);
...
Type.registerNamespace("Telerik.Web.UI.ImageEditor");
(function(a,c,b,f){var d=Telerik.Web.Browser;
b.DraggableResizeBox=function(g){this.set_element(g);
this._cropBounds=null;
this._boxElement=null;
this._createUI();
this.scrollSettings={scrollEdgeConstSmall:20,scrollEdgeConst:30,scrollSpeed:2};
};
b.DraggableResizeBox.prototype={_createUI:function(){var g=a("<div class='rieDraggableResizeBox'>");
this._appendHandle(g);
g.append(a("<div class='rieDraggableResizeHandle rieTop rieLeft'>"));
g.append(a("<div class='rieDraggableResizeHandle rieTop rieMiddleH'>"));
g.append(a("<div class='rieDraggableResizeHandle rieTop rieRight'>"));
g.append(a("<div class='rieDraggableResizeHandle rieLeft rieMiddleV'>"));
g.append(a("<div class='rieDraggableResizeHandle rieRight rieMiddleV'>"));
g.append(a("<div class='rieDraggableResizeHandle rieBottom rieLeft'>"));
g.append(a("<div class='rieDraggableResizeHandle rieBottom rieMiddleH'>"));
g.append(a("<div class='rieDraggableResizeHandle rieBottom rieRight'>"));
this._jBoxElement=g;
this._boxElement=g[0];
this.get_element().appendChild(this._boxElement);
this._storeParentOffset();
this._setInitialPosition();
this._makeResizeable();
},_appendHandle:function(g){g.append(a("<div class='rieDraggableBoxMove'>"));
},_makeResizeable:function(){this.disposeResizeExtender();
if(this._boxElement){this._createResizeExtender();
}},_createResizeExtender:function(){var h=a(this._boxElement);
var g={nw:h.find(".rieTop.rieLeft",this)[0],n:h.find(".rieTop.rieMiddleH",this)[0],ne:h.find(".rieTop.rieRight",this)[0],w:h.find(".rieLeft.rieMiddleV",this)[0],e:h.find(".rieRight.rieMiddleV",this)[0],sw:h.find(".rieBottom.rieLeft",this)[0],s:h.find(".rieBottom.rieMiddleH",this)[0],se:h.find(".rieBottom.rieRight",this)[0],move:h.find(".rieDraggableBoxMove")[0]};
this._resizeExtender=new c.ResizeExtender(this,this._boxElement,g);
this._overrideResizeExtenderMethods();
this._resizeExtenderScroller=new b.ResizeExtenderScroller(this._resizeExtender);
this._resizeExtenderScroller.add_scrolling(a.proxy(this._onScrolling,this));
},_setInitialPosition:function(){var g=a(this.get_element());
this.setBounds({x:g.scrollLeft(),y:g.scrollTop(),width:50,height:50});
},_storeParentOffset:function(){var g=("relative"==$telerik.getCurrentStyle(this.get_element(),"position"));
this._offsetLocation=(g?$telerik.getLocation(this.get_element()):null);
},_raiseResizeEvent:function(i){var g=new Sys.EventArgs();
var h=this.getBounds();
g.getBounds=function(){return h;
};
g.get_value=function(){return i;
};
this.raiseEvent("boundsChanged",g);
},onResizing:function(g){return this._raiseResizeEvent(g);
...
return this._raiseResizeEvent();
},onDrag:function(g){this._manageScroller(g);
return this._raiseResizeEvent();
},onDragStart:function(g){$telerik.cancelRawEvent(g.ownerEvent);
return this._resizeExtenderScroller.startScrolling();
},onResizeStart:function(g){$telerik.cancelRawEvent(g.ownerEvent);
return this._resizeExtenderScroller.startScrolling();
...
this._resizeExtender=null;
}},setVisible:function(i){var g=this._boxElement;
g.style.visibility=i?"visible":"hidden";
var h=$telerik.getScrollOffset(g.parentNode);
$telerik.setLocation(g,h);
},getBounds:function(){var g=this.getPosition();
var h=this.getSize();
return new Telerik.Web.UI.Bounds(g.left,g.top,h.width,h.height);
},getPosition:function(){var h=this._jBoxElement.position();
var g=this._jBoxElement.parent();
var i={left:g.scrollLeft(),top:g.scrollTop()};
return{left:h.left+i.left,top:h.top+i.top};
},getSize:function(){return{width:this._jBoxElement.outerWidth(),height:this._jBoxElement.outerHeight()};
},setBounds:function(g){if(g.width<2){g.width=2;
}if(g.height<2){g.height=2;
}$telerik.setBounds(this._boxElement,g);
},get_constraints:function(){return this._resizeExtender.get_constraints();
},set_constraints:function(g){this._resizeExtender.set_constraints(g);
g.horizontal=g.x+g.width;
g.vertical=g.y+g.height;
},get_maxWidth:function(){var g=this.getBounds();
var h=this.get_constraints();
return h.width-g.x;
},get_maxHeight:function(){var g=this.getBounds();
var h=this.get_constraints();
return h.height-g.y;
},add_boundsChanged:function(g){a(this).bind("boundsChanged",g);
},remove_boundsChanged:function(g){a(this).unbind("boundsChanged",g);
},raiseEvent:function(h,g){a(this).trigger(h,[g]);
},get_sizeRatio:function(){return this._ratio;
},set_sizeRatio:function(g){this._ratio=g;
this._resizeExtender._ratio=g;
...
},_resizeExtenderResizeMethod:function(j){if(!this._enabled||this._cancelResize){return false;
}if(d.ie&&d.version==10&&j.clientX===f){j=j.originalEvent;
}var r=this._jsOwner._boxElement.parentNode;
var q=e(r)?$telerik.getLocation(r):{x:0,y:0};
var g=this._originalBounds;
var p=new Telerik.Web.UI.Bounds(0,0,0,0);
var l=($telerik.isMobileSafari||$telerik.isAndroid)?$telerik.getTouchEventLocation(j):{x:j.clientX,y:j.clientY};
var w=l.x-this._startX;
var x=l.y-this._startY;
var i=this._resizeDir;
var m=i.move;
if(m){var n=this._startCursorLocation;
if(n){g.x=n.x;
g.y=n.y;
...
p.y=g.y+x;
var o=this._getMoveConstraints(g);
if(o){p.x=this._constrainPosition(p.x,o.x,o.width);
p.y=this._constrainPosition(p.y,o.y,o.height);
}}else{if(i.east){p.x=g.x;
p.width=g.width+w;
}else{if(i.west){p.x=l.x-this._leftHandleMouseDelta-q.x;
p.width=g.width-w;
}}if(i.south){p.y=g.y;
p.height=g.height+x;
}else{if(i.north){p.y=g.y+x;
p.height=g.height-x;
}}var v=this._getSizeConstraints(g);
if(v){p.x=p.x||g.x;
p.y=p.y||g.y;
if(i.west){p.x=this._constrainPosition(p.x,v.x,Math.min(p.x+p.width,v.width-p.width));
}if(i.north){p.y=this._constrainPosition(p.y,v.y,Math.min(p.y+p.height,v.height-p.height));
}v.width-=p.x+this._borderDims.horizontal;
v.height-=p.y+this._borderDims.vertical;
p.width=this._constrainDimension(p.width,v.width);
p.height=this._constrainDimension(p.height,v.height);
}if(this._ratio){var s=p.width/this._ratio;
var t=p.height*this._ratio;
if(i.east||i.west){p.height=v?this._constrainDimension(s,v.height):s;
p.width=p.height*this._ratio;
}else{p.width=v?this._constrainDimension(t,v.width):t;
p.height=p.width/this._ratio;
}}}var u=m?this._raiseDragEvent("Drag",p,j):this._raiseEvent("Resizing",p);
if(false==u){return true;
}var k=this._element;
if(this._useCssTransform&&m){this._lastTouchDelta={x:p.x-g.x,y:p.y-g.y};
var h={x:p.x?this._lastTouchDelta.x:0,y:p.y?this._lastTouchDelta.y:0};
this._setTranslate(k,h.x,h.y);
}else{if(m||p.x>0){k.style.left=p.x+"px";
}if(m||p.y>0){k.style.top=p.y+"px";
}}if(p.width>0){k.style.width=p.width+"px";
}if(p.height>0){k.style.height=p.height+"px";
}return true;
},_manageScroller:function(g){this._resizeExtenderScroller.manageScrolling(g);
...
}}},_onScrolling:function(h,g){this._resizeExtender._startX-=g.newScroll.left-g.oldScroll.left;
this._resizeExtender._startY-=g.newScroll.top-g.oldScroll.top;
...
b.DraggableImageResizeBox=function(g){this._imageElement=null;
this._jImageElement=null;
this.IsImageBroken=false;
b.DraggableImageResizeBox.initializeBase(this,[g]);
};
...
},_appendHandle:function(g){var h=a("<div class='rieDraggableBoxMove rieDraggableBoxImage'>");
this._jImageElement=a("<img alt='Insert clip art' style='width:100%; height:100%;'>");
h.append(this._jImageElement);
g.append(h);
this._imageElement=this._jImageElement[0];
this.attachHandlers(true);
},_imageLoadHandler:function(g){this.IsImageBroken=false;
this._setInitialSize();
},_imageErrorHandler:function(g){this.IsImageBroken=true;
...
},attachHandlers:function(h){var g=this._imageElement;
if(h){this._imageLoadDelegate=Function.createDelegate(this,this._imageLoadHandler);
$addHandler(g,"load",this._imageLoadDelegate);
this._imageErrorDelegate=Function.createDelegate(this,this._imageErrorHandler);
if($telerik.useAttachEvent(g)){g.attachEvent("onerror",this._imageErrorDelegate);
}else{if(g.addEventListener){g.addEventListener("error",this._imageErrorDelegate,false);
}}}else{$removeHandler(g,"load",this._imageLoadDelegate);
delete this._imageLoadDelegate;
if($telerik.useDetachEvent(g)){g.detachEvent("onerror",this._imageErrorDelegate);
}else{if(g.removeEventListener){g.removeEventListener("error",this._imageErrorDelegate,false);
}}delete this._imageErrorDelegate;
}},set_src:function(g){this._jImageElement.attr("src",g);
this._setInitialSize();
},get_src:function(){if(this._jImageElement){return this._jImageElement.attr("src");
}},setSize:function(g){if(g.width<2){g.width=2;
}if(g.height<2){g.height=2;
}$telerik.setSize(this._boxElement,g);
...
function e(g){var h=$telerik.getComputedStyle(g,"position");
return h==="relative"||h==="absolute";
}})($telerik.$,Telerik.Web.UI,Telerik.Web.UI.ImageEditor);
Type.registerNamespace("Telerik.Web.UI.ImageEditor");
(function(a,c,b){b.DraggableTextBox=function(d){this.set_container(d);
...
},set_text:function(g){this._textSettings.set_text(g);
try{var f=this._formatTextToHtml();
this._boxContent.html(f);
this._updateTextSize();
}catch(d){}},get_textSettings:function(){return this._textSettings;
...
},set_lineHeight:function(d){this._textSettings.set_lineHeight(d);
},get_bounds:function(){var d=this._boxElement;
return new Telerik.Web.UI.Bounds(parseInt(d.css("left"),10),parseInt(d.css("top"),10),parseInt(d.css("width"),10),parseInt(d.css("height"),10));
},set_bounds:function(d){this.set_position(d);
this.set_size(d);
},get_size:function(){var d=this._boxElement;
return{width:parseInt(d.css("width"),10),height:parseInt(d.css("height"),10)};
},set_size:function(d){this._boxElement.css({width:d.width,height:d.height});
},get_position:function(){var d=this._boxElement;
return{x:parseInt(d.css("left"),10),y:parseInt(d.css("top"),10)};
...
},add_boundsChanged:function(d){a(this).bind("boundsChanged",d);
},remove_boundsChanged:function(d){a(this).unbind("boundsChanged",d);
},onDrag:function(d){var e=this.get_container();
d.x+=e.scrollLeft;
d.y+=e.scrollTop;
a(this).trigger("boundsChanged");
this._resizeExtenderScroller.manageScrolling(d);
},onDragEnd:function(){this._resizeExtenderScroller.stopScrolling();
a(this).trigger("boundsChanged");
...
},_createResizeExtender:function(){this._resizeExtender=new c.ResizeExtender(this,this._boxElement[0],{move:this._boxContent[0]});
this._resizeExtenderScroller=new b.ResizeExtenderScroller(this._resizeExtender);
...
d.width+=parseInt(this._boxElement.css("padding-left"),10)+parseInt(this._boxElement.css("padding-right"),10)+1;
d.height+=parseInt(this._boxElement.css("padding-top"),10)+parseInt(this._boxElement.css("padding-bottom"),10)+1;
return d;
},_setVisible:function(d){this._boxElement.css({visibility:d});
},_formatTextToHtml:function(){return this.get_text().replace(/\n/g,"<br/>");
},onDragStart:function(d){$telerik.cancelRawEvent(d.ownerEvent);
return this._resizeExtenderScroller.startScrolling();
}};
...
Type.registerNamespace("Telerik.Web.UI.ImageEditor");
(function(a,c,b){b.ResizeExtenderScroller=function(e,d){this._resizeExtender=e;
...
},setOptions:function(d){this.options=a.extend({scrollEdgeConstSmall:-4,scrollEdgeConst:-4,scrollSpeed:2,constraints:null},d||{});
...
var f=e.parent();
this._scrollerX=this._calculateScrollDelta(d.x,e.width(),f.width(),f.scrollLeft());
this._scrollerY=this._calculateScrollDelta(d.y,e.height(),f.height(),f.scrollTop());
this._initScrolling();
...
},_scrollViewport:function(){var e=a(this.element);
var d=this.options.bounds;
var g=e.parent();
var h=this._getScrollPosition(g);
g.scrollLeft(h.left+this._scrollerX);
g.scrollTop(h.top+this._scrollerY);
var f=this._getScrollPosition(g);
this.manageScrolling(d);
this.raiseEvent("scrolling",{oldScroll:h,newScroll:f});
...
},_calculateScrollDelta:function(g,h,i,d){var e=this._resizeExtender._resizeDir;
var f=e.move||e.west||e.north;
if(f&&g<d+this.options.scrollEdgeConstSmall){return 0-this.options.scrollSpeed;
}else{if(g+h+this.options.scrollEdgeConst>d+i){return this.options.scrollSpeed;
}else{return 0;
