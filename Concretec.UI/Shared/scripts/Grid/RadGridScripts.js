...
if(h.findItemByValue("GroupBy")){s=h.findItemByValue("GroupBy");
if(k._data.Groupable){h.findItemByValue("GroupBy")._column=k;
s.set_visible(true);
}else{s.set_visible(false);
}}if(h.findItemByValue("UnGroupBy")){s=h.findItemByValue("UnGroupBy");
...
window.$addHandler(n,"click",this._filterButtonClickDelegate);
var d=$telerik.findElement(h.findItemByValue("FilterMenuContainer").get_element(),"HCFMClearFilterButton");
d.onclick="";
this._clearFilterButtonClickDelegate=Function.createDelegate(this,this._clearFilterButtonClickHandler);
window.$addHandler(d,"click",this._clearFilterButtonClickDelegate);
...
if(q&&q.length&&q[0].type=="checkbox"){window.$addHandler(q[0],"click",this._checkBoxClickHandler);
if(f.get_visible()){q[0].checked=true;
...
if(y){var v=y.x;
var B=y.y;
if(z){v=v+parseInt(z,10);
}if(A){B=B+parseInt(A,10);
}h.showAt(v,B);
...
var q=c.findItemByValue("FilterMenuContainer").findControl(p).get_selectedItem().get_value();
var k;
var r;
var s=true;
...
if(!this._owner._owner._selection._selectRowInternal(this.get_element(),a,false,true,true)){return;
...
},_initializeDimensions:function(){var e=this;
this.onWindowResize();
this.initializeAutoLayout();
this.applyFrozenScroll();
var d=0;
this._onResizeDelegate=Function.createDelegate(this._owner,function(){var f=this;
clearInterval(d);
d=setTimeout(function(){clearInterval(d);
f.repaint();
},200);
});
if(navigator.userAgent.toLowerCase().indexOf("msie")!=-1){setTimeout(function(){window.$addHandler(window,"resize",e._onResizeDelegate);
},0);
...
eval(m);
}}}},_selectRowInternal:function(s,h,m,x,v,t){var u;
...
},resizeColumn:function(t,E){var F=false;
var m=this._hasMultiHeaders;
var z=0;
var v;
if(($telerik.isIE7||$telerik.isIE6)&&!$telerik.quirksMode){var n=this.get_columns()[t].get_element();
z=parseInt($telerik.getCurrentStyle(n,"borderLeftWidth",0),10)+parseInt($telerik.getCurrentStyle(n,"paddingLeft",0),10)+parseInt($telerik.getCurrentStyle(n,"paddingRight",0),10)+parseInt($telerik.getCurrentStyle(n,"borderRightWidth",0),10);
if(E>z){E-=z;
F=true;
}}if(!this._validateResizeColumnParams(t,E)){return;
...
if(a.get_cancel()){return false;
}var x=this.get_element().offsetWidth;
var w=this._owner.get_element().offsetWidth-parseInt($telerik.getCurrentStyle(this._owner.get_element(),"borderLeftWidth",0),10)-parseInt($telerik.getCurrentStyle(this._owner.get_element(),"borderRightWidth",0),10);
if(this==this._owner.get_masterTableView()&&this._owner.get_masterTableViewHeader()){this._owner.get_masterTableViewHeader().resizeColumn(t,E);
}var f;
var p;
...
for(v=0;
v<p.length;
v++){if(p[v].style.display=="none"){r++;
}else{D++;
if(D==t+1){break;
}}}e=parseInt(r+D,10)-1;
}f=p[e].offsetWidth-E;
if(($telerik.isIE7||$telerik.isIE6)&&!$telerik.quirksMode&&F){f-=z;
...
if(g){if(g.cells[t]){g.cells[t].style.width=E+"px";
}}if(this._owner.get_masterTableViewFooter()&&this._owner.get_masterTableViewFooter().get_element()){if(this._owner.get_masterTableViewFooter().get_element().tBodies[0].rows[0]&&this._owner.get_masterTableViewFooter().get_element().tBodies[0].rows[0].cells[t]){if(E>0){this._owner.get_masterTableViewFooter().get_element().tBodies[0].rows[0].cells[t].style.width=E+"px";
}}}}}if(this.ColGroup){if(this.ColGroup.Cols[t]){if(E>0){var u=(this._owner.ClientSettings.Scrolling&&this._owner.ClientSettings.Scrolling.AllowScroll&&this._owner.ClientSettings.Scrolling.UseStaticHeaders);
if(!this._colWidthsCorrected){var y=Telerik.Web.UI.Grid.getMultiHeaderCells(this);
var C=this.get_element();
if((C.id.indexOf("_Header")!=-1&&u&&($telerik.isIE6||$telerik.isIE7))){var q=C.tHead?C.tHead.rows[0]:null;
if(q&&q.className&&q.className.indexOf("rgMultiHeaderRow")>-1){var b=q.cells;
var c=C.getElementsByTagName("colgroup")[0];
...
v++){d[v].level=B;
d[v].corrected=false;
if(o.colSpan>1&&v>=B+1&&d[v]&&y[v]){d[v].corrected=true;
this._colWidthsCorrected=true;
}}B+=o.colSpan;
}}}}}if(($telerik.isIE7||$telerik.isIE6)&&!$telerik.quirksMode&&this.ColGroup.Cols[t].corrected&&this._owner.get_masterTableViewHeader()&&(this.get_id()==this._owner.get_masterTableViewHeader().get_id())){this.ColGroup.Cols[t].style.width=(E+z)+"px";
}else{this.ColGroup.Cols[t].style.width=E+"px";
}}}}if(this._owner.get_masterTableViewHeader()&&(this.get_id()==this._owner.get_masterTableViewHeader().get_id())){if(this._owner.get_masterTableView().ColGroup){if(this._owner.get_masterTableView().ColGroup.Cols[t]){if(E>0){this._owner.get_masterTableView().ColGroup.Cols[t].style.width=E+"px";
}}}if(this._owner.get_masterTableViewFooter()&&this._owner.get_masterTableViewFooter().ColGroup){if(this._owner.get_masterTableViewFooter().ColGroup.Cols[t]){if(E>0){this._owner.get_masterTableViewFooter().ColGroup.Cols[t].style.width=E+"px";
}}}}if(E.toString().indexOf("px")!=-1){E=E.replace("px","");
}if(E.toString().indexOf("%")==-1){E=E+"px";
}this._resizedColumnsData.add(String.format("{0},{1}",this._data.UniqueID,this.get_columns()[t].get_uniqueName()),E);
...
}if(this._owner.ClientSettings.Resizing.ResizeGridOnColumnResize){if(f!=null){this._resizeGridOnColumnResize(t,f,x);
}}else{this._noResizeGridOnColumnResize(x,t,w);
...
}var A=this._scrollbarHeight;
if(k.offsetWidth>=this._owner.GridDataDiv.clientWidth){if($telerik.isIE){A++;
}k.style.height=A+"px";
...
if(this._isRightToLeft&&this._isScrollOnLeftSide){k.style.marginLeft=A+"px";
}else{if(!this._isRightToLeft&&$telerik.isSafari){k.style.marginRight=A+"px";
...
}}if(a){if(this.get_owner()._renderMode==Telerik.Web.UI.RenderMode.Lite){k.className="rgActionButton rgExpand";
...
}else{if(this.get_owner()._renderMode==Telerik.Web.UI.RenderMode.Lite){k.className="rgActionButton rgCollapse";
...
for(d=0;
d<b.length;
d++){if(c!=b[d].get_expanded()){f=false;
break;
}}if(f){var e=this.get_owner()._renderMode==Telerik.Web.UI.RenderMode.Lite;
for(d=0;
d<a.length;
d++){if(c){if(e){a[d].className="rgActionButton rgCollapse";
if(a[d].children.length>0&&a[d].children[0].tagName.toLowerCase()=="span"){a[d].children[0].className="rgIcon rgCollapseIcon";
}}else{a[d].className="rgCollapse";
}}else{if(e){a[d].className="rgActionButton rgExpand";
if(a[d].children.length>0&&a[d].children[0].tagName.toLowerCase()=="span"){a[d].children[0].className="rgIcon rgExpandIcon";
...
e<b.length;
e++){var d=b[e].get_element();
if(d.className.indexOf("rgExpandCol")>-1){var c=f?d.getElementsByTagName("button")[0]:d.getElementsByTagName("input")[0];
Array.add(a,c);
}}return a;
...
if(M&&M.className.indexOf("rgIcon")>-1){M.className=M.className.replace(/Expand/g,"Collapse");
}}if(this._owner.get_enableAriaSupport()){o.setAttribute("aria-expanded","true");
o.parentNode.parentNode.setAttribute("aria-expanded","true");
...
if(M&&M.className.indexOf("rgIcon")>-1){M.className=M.className.replace(/Collapse/g,"Expand");
}}if(this._owner.get_enableAriaSupport()){o.setAttribute("aria-expanded","false");
o.parentNode.parentNode.setAttribute("aria-expanded","false");
}}}}}}var m="none";
...
v<N.rows.length;
v++){G=N.rows[v];
if(Q&&!w&&this._shouldSkipOtherRows(G,Q)){continue;
...
for(A=0;
A<b;
A++){G=N.rows[N.rows.length-A-1];
if(!G.id){G.style.display="";
}}}if(o===D&&u){var p=this._owner._expandedGroupItems,B=O._data.UniqueID+"!"+J+"!"+t;
l=0;
for(var r=0;
r<p.length;
r++){if(p[r]==B){l++;
...
}if(this.get_masterTableView().get_allowPaging()&&$telerik.isTouchDevice){var p=this.ClientSettings.Scrolling.AllowScroll?this.GridDataDiv:$telerik.getChildrenByTagName(this.get_masterTableView().get_element(),"tbody")[0];
...
if(n){var o=this._selectedItemsInternal;
for(h=0;
h<o.length;
h++){Array.add(this._selectedIndexes,o[h].itemIndex);
...
if(this.ClientSettings.AllowKeyboardNavigation&&q!=null&&q!=""){setTimeout(function(){try{var s=false;
var i=$find(q);
if(i==null){i=$get(q);
}else{s=true;
}if(i==null){i=document.getElementsByName(q.replace(/_/ig,"$"))[0];
}if(i!=null){if(i.focus){i.focus();
}else{if(s){if(i._focused!=undefined){i._focused=true;
...
var n=this;
if(navigator.userAgent.toLowerCase().indexOf("msie")!=-1){setTimeout(function(){window.$addHandler(window,"resize",n._onResizeDelegate);
},0);
}else{window.$addHandler(window,"resize",this._onResizeDelegate);
...
if(k.offsetHeight!=0){k.getElementsByTagName("div")[4].style.height=k.offsetHeight-b.offsetHeight+"px";
}this._popUpLocations[b.id]=parseInt(k.style.left,10)+"px,"+parseInt(k.style.top,10)+"px";
this.updateClientState();
window.$addHandlers(b,{mousedown:Function.createDelegate(k,this._popUpMouseDown)});
window.$addHandlers(document,{mouseup:Function.createDelegate(k,this._popUpMouseUp)});
window.$addHandlers(document,{mouseout:Function.createDelegate(k,this._popUpMouseOut)});
if(this.ClientSettings.AllowKeyboardNavigation&&this.ClientSettings.KeyboardNavigationSettings.EnableKeyboardShortcuts){window.$addHandler(k,"keypress",Function.createDelegate({popUpForm:k,keyMappings:this.ClientSettings.KeyboardNavigationSettings},this._popUpKeyDown));
...
}}if(this._masterClientID&&this.get_masterTableView()&&this.get_masterTableView()._virtualization){this.get_masterTableView()._virtualization.repaint();
...
if(this.ClientSettings.Selecting.AllowRowSelect&&this.get_allowMultiRowSelection()){G.setAttribute("aria-multiselectable","true");
}var z=this.get_masterTableView();
var k=z.get_columns();
var p=z._data.EditMode;
var D=z.get_dataItems();
var t;
var x;
var y;
var A;
...
if(f&&f.AllowRowsDragDrop&&f.Selecting&&f.Selecting.AllowRowSelect){var C=A.get_element();
C.setAttribute("aria-dropeffect","move");
}for(x=0;
x<k.length;
x++){var g=k[x];
var o=null;
...
}var F=g.get_uniqueName();
if(g._data.ColumnType=="GridEditCommandColumn"&&p=="PopUp"){o=F;
}a=A.get_cell(F);
...
}if(o&&p=="PopUp"){if(a&&a.firstChild&&(a.firstChild.tagName.toLowerCase()=="img"||a.firstChild.tagName.toLowerCase()=="a"||a.firstChild.tagName.toLowerCase()=="input")){a.firstChild.setAttribute("aria-haspopup","true");
}}}}if(this._detailTables.length){G.setAttribute("role","treegrid");
}else{G.setAttribute("role","grid");
}var u=function(i){i.setAttribute("role","columnheader");
...
if(G.querySelectorAll){s=G.querySelectorAll("th.rgHeader");
for(t=0,y=s.length;
t<y;
t++){u(s[t]);
}}else{s=G.getElementsByTagName("th");
for(t=0,y=s.length;
t<y;
t++){var r=s[t];
if(r.className.indexOf("rgHeader")>-1){u(r);
}}}var b=G.getElementsByTagName("td");
for(t=0,y=b.length;
t<y;
t++){a=b[t];
A=a.parentNode;
...
if(B.indexOf("rgGroupHeader")>-1||A.id){var q=a.getElementsByTagName("input")[0];
if(q){if(q.className.indexOf("rgCollapse")>-1){A.setAttribute("aria-expanded","true");
q.setAttribute("aria-expanded","true");
}else{if(q.className.indexOf("rgExpand")>-1){A.setAttribute("aria-expanded","false");
q.setAttribute("aria-expanded","false");
}}}if(B.indexOf("rgGroupHeader")>-1&&!A.getAttribute("role")){A.setAttribute("role","rowgroup");
}}}else{if(B.indexOf("rgRow")>-1||B.indexOf("rgAltRow")>-1||B.indexOf("rgFooter")>-1){a.setAttribute("role","gridcell");
if(!A.getAttribute("role")){A.setAttribute("role","row");
if(B.indexOf("rgSelectedRow")>-1){A.setAttribute("aria-selected","true");
A.tabIndex=1;
}}}else{if(a.className.indexOf("rgCommandCell")>-1&&!A.getAttribute("role")){a.setAttribute("role","presentation");
A.setAttribute("role","presentation");
}else{if(A.className.indexOf("rgPager")>-1&&!A.getAttribute("role")){A.setAttribute("role","presentation");
var n=A.getElementsByTagName("div");
for(var l=0;
l<n.length;
l++){var m=n[l];
if(m.className&&m.className.indexOf("rgWrap")>-1){var w=m.getElementsByTagName("input");
for(x=0;
x<w.length;
x++){var v=w[x];
var e=v.className;
if(v.type=="submit"&&e&&(e.indexOf("PagePrev")>-1||e.indexOf("PageFirst")>-1||e.indexOf("PageNext")>-1||e.indexOf("PageLast")>-1||e.indexOf("PagerButton")>-1)){v.setAttribute("role","button");
}}}}}else{if(A.className.indexOf("rgEditRow")>-1&&!A.getAttribute("role")){A.setAttribute("role","row");
A.setAttribute("aria-readonly","false");
}else{if(!A.id){var E=A.parentNode.tagName.toLowerCase()==="table"?A.parentNode:A.parentNode.parentNode;
if(E&&E.className.indexOf("rgMasterTable")>-1||E.className.indexOf("rgDetailTable")>-1){if(!A.getAttribute("role")){A.setAttribute("role","presentation");
...
}},_showFilterMenu:function(k,m,c){var j=$find(k);
var b=j.getColumnByUniqueName(m);
var i=this._getFilterMenu();
if(i){i.hide();
var a=new Sys.CancelEventArgs();
a.get_menu=function(){return i;
};
a.get_tableView=function(){return j;
};
...
if($telerik.isTouchDevice){var h=$telerik.getTouchEventLocation(c).x;
var l=$telerik.getTouchEventLocation(c).y;
this._filterMenu.showAt(h,l);
$telerik.cancelRawEvent(c);
}else{var f=$telerik.getLocation(d);
this._filterMenu.showAt(f.x+(d.offsetWidth/2),f.y+(d.offsetHeight/2));
$telerik.cancelRawEvent(c);
var g=this._filterMenu.get_focusedItem()||this._filterMenu.get_items().getItem(0);
if(c.clientX==0&&c.clientY==0){if(g._linkElement){g._linkElement.focus();
}else{if(g._templateElement){g._templateElement.focus();
...
if(h&&k||!f){this._shouldFocusOnPage=true;
