...
a("div.rsTopWrap div.rsHeader h2, div.rsTopWrap div.rsToolbar .rsCurrent",this._ownerElement).text(o);
...
if(Sys.CultureInfo.CurrentCulture&&Sys.CultureInfo.CurrentCulture.name=="en-US"){switch(q){case"dd":if(p<10){return"0"+p;
}else{return p;
}break;
...
for(var D=0,z=C/v;
D<z;
D++){var F=[],q=b.add(y.get_visibleRangeStart(),h*D*l),G=e.DateHelper.getStartOfWeek(q,this._owner.get_firstDayOfWeek(),E);
for(var s=0;
s<E;
...
for(var z=0,w=y/u;
z<w;
z++){var p=b.add(v.get_visibleRangeStart(),h*z*l),B=e.DateHelper.getStartOfWeek(p,this._owner.get_firstDayOfWeek(),A);
for(var r=0;
r<A;
