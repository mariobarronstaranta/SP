...
},loadAppointments:function(k){if(this._suppressWebServiceCalls){return;
}var j=this,i=this._isWebApi,h=this._getSchedulerInfo(k),g;
if(!this._onAppointmentsPopulating(h)){return;
}if(i){h=Sys.Serialization.JavaScriptSerializer.serialize(h);
}g={methodName:j._settings.get_method(),data:{schedulerInfo:h},context:"loadAppointments",httpVerb:c.HTTPVerbs.GET};
...
return !g.get_cancel();
},_onRequestSucceeded:function(k,h){var i=this._owner;
var j=h.get_data();
var g=j.Appointments||j;
i._reminders=[];
i._raiseRequestSuccess(new d.RequestSuccessEventArgs(j));
this._suppressWebServiceCalls=true;
if(h.get_context()=="loadResources"){this._loadResources(i.get_resources(),g);
this._resourcesLoaded=true;
i._raiseResourcesPopulated();
}else{this._raiseEvent("appointmentsReceived");
this._populateAppointments(g);
this._raiseEvent("appointmentsPopulated");
this._appointmentsLoaded=true;
}if(this._appointmentsLoaded&&this._resourcesLoaded){i._raiseDataBound();
}this._suppressWebServiceCalls=false;
if(i._reminderDialog&&a(i._reminderDialog.get_element()).is(":visible")){i._pollReminders();
}if(this._shouldTriggerDismiss){i._dismissRemindersWS(i._reminders);
...
},_populateAppointments:function(h){var j=[];
var m,k;
var n=this._owner;
var o=n.get_activeModel()._renderer;
var p=n.get_appointments();
for(m=0,k=h.length;
m<k;
m++){var g=this._loadAppointment(h[m]);
Array.add(j,g);
var l=new d.AppointmentDataBoundEventArgs(g,h[m]);
n._raiseAppointmentDataBound(l);
}j=j.concat(p._array);
j.sort(function(i,q){return i.compare(q);
});
p._clear();
n._batchRendering=true;
for(m=0,k=j.length;
m<k;
m++){p.add(j[m]);
}n._batchRendering=false;
if(o._recalcAppointmentsStyles){o._recalcAppointmentsStyles();
}n.repaint();
