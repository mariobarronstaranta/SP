...
a(j).css("opacity",0.6);
if($telerik.isIE&&m.get_renderMode()!==b.RenderMode.Lite){a(j).css("padding-bottom","4px");
if($telerik.isIE6){var l=a(".rsAptResize",j);
...
},_resetDragStyles:function(i){var k=this.get_owner();
var h=k._initialDragAppointmentElement;
a(h).css("opacity","");
if($telerik.isIE&&k.get_renderMode()!==b.RenderMode.Lite){a(h).css("padding-bottom","0px");
if($telerik.isIE6){var j=a(".rsAptResize",h);
if(j.length>0){j.css("opacity",j.data("opacity"));
}}}if(i&&k.get_selectedView()==b.SchedulerViewType.TimelineView){h.style.left=this._originalLeftOffset;
h.style.marginLeft=this._originalLeftMargin;
}h.style.zIndex="";
var l=k.get_element();
if(l&&l.style){l.style.cursor="";
...
k.css(m,n);
}if($telerik.isIE6||$telerik.quirksMode){if(l){v+=n;
}k.css("margin-left",v+"px");
}else{k.scrollLeft(v);
...
a(n.contentWrapper).find("table.rsContentTable, table.rsAllDayTable, table.rsAgendaTable, div.rsSubHeader:visible").each(function(){if(a(this).parents().is(".rsHorizontal")){q=a(this).outerHeight();
...
var m=this._getCellFromCoordinates(l.x,l.y-j);
return m;
